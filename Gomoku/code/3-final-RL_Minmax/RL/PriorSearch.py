...
class PriorSearch():
    ...
    def VcfStart(self): 
        ...
        for type in [VCF6,VCF5,VCF4,VCF3,VCF2,VCF1]:
            if self.board.Cross[self.board.my][type] : 
                return list(self.board.Cross[self.board.my][type])[0]
        return -1
    ...
    def Vcf2(self,searcher,depth,startpoint):
        ...
        if self.board.Cross[self.board.my][VCF6] : 
            ...
            return 1
        if len(self.board.Cross[self.board.opp][VCF6]) >= 2: return -2
        ...
        if self.board.my == searcher and self.board.Cross[self.board.my][VCF4]:
            # 对方没有能成四的点，五步胜
            if not (self.board.Cross[self.board.opp][VCF5] | self.board.Cross[self.board.opp][VCF4] | self.board.Cross[self.board.opp][VCF3] | self.board.Cross[self.board.opp][VCF2] | self.board.Cross[self.board.opp][VCF1]):
        ...
        if self.board.my == searcher and depth < MAXFDEPTH and (self.board.Cross[self.board.my][VCF3] | self.board.Cross[self.board.my][VCF2]): 
            for point in product(range(startpoint[0]-4,startpoint[0]+5),range(startpoint[1]-4,startpoint[1]+5)):
    ...
    def Vcf3(self, searcher, depth, startpoint):
        # 己方有即将连五，1步赢     
        if self.board.Cross[self.board.my][VCF6]: return 1   
        # 对方有双连五，2步输
        if len(self.board.Cross[self.board.opp][VCF6]) >= 2: return -2
        # offend
        if self.board.Cross[self.board.opp][VCF6]:
            ...
            return q
        # 己方活四，三步胜利
        if self.board.Cross[self.board.my][VCF5]: return 3
        # 己方有冲四活三
        if self.board.my == searcher and self.board.Cross[self.board.my][VCF4]:
            # 对方没有活四冲四，五步胜利；其他情况不予考虑（返回0）
            if not (self.board.Cross[self.board.opp][VCF5] | self.board.Cross[self.board.opp][VCF4] | self.board.Cross[self.board.opp][VCF3] | self.board.Cross[self.board.opp][VCF2] | self.board.Cross[self.board.opp][VCF1]): return 5
            for point in self.board.Cross[self.board.my][VCF4]:
                self.board.move(point[0],point[1])
                q = -self.Vcf3(searcher, depth+1, point)
                self.board.delmove()
...
################# VCT Start ###################
    ...
    def VctStart(self):
        if self.board.step < 4:  return -1

        for type in [VCT4,VCT3,VCT2,VCT1]:
            if self.board.Cross[self.board.my][type] : 
                return list(self.board.Cross[self.board.my][type])[0]
    ...
    def Vct1(self):
        ...
        if startpoint != -1:
            for depth in range(10, MAXTDEPTH + 2, 2):
                ...
                if result > 0 or 4000 * (time() - VCT_starttime) >= MAXTTIME: break
            return result
    ...
    def Vct2(self, searcher, depth, maxdepth, startpoint):
        ...
        if self.board.my != searcher and self.board.Cross[self.board.opp][VCF5]:
            max_q = -1000
            cands = self.board.getCandidates()
            for cand in cands:
                self.board.move(cand[0], cand[1])
                q = -self.Vct3(searcher, depth+1, maxdepth, startpoint)
                self.board.delmove()
                if q>0:
        ...
        # 己方先手，遍历所有冲四的点 除冲四活三
        if self.board.my == searcher and (self.board.Cross[self.board.my][VCF3] | self.board.Cross[self.board.my][VCF2]): 
            for point in product(range(startpoint[0]-4,startpoint[0]+5),range(startpoint[1]-4,startpoint[1]+5)):
                if point in (self.board.Cross[self.board.my][VCF3] | self.board.Cross[self.board.my][VCF2]):
                    self.board.move(point[0],point[1])
                    q = -self.Vct3(searcher, depth+1, maxdepth, point)
        ...
        if self.board.my == searcher and self.board.Cross[self.board.my][VCT4]:
            ...
            for point in self.board.Cross[self.board.my][VCT4]:
                ...
                if q > 0:
                    ...
                    return q+1
        # 遍历其他情况
        if self.board.my == searcher and (self.board.Cross[self.board.my][VCT3] | self.board.Cross[self.board.my][VCT2]):
            for point in product(range(startpoint[0]-4,startpoint[0]+5),range(startpoint[1]-4,startpoint[1]+5)):
