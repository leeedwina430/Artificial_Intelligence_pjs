...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                mean = math.sqrt((agentX-util.colToX(col))**2 + (agentY-util.rowToY(row))**2)
                # P(dt | ct)
                p = util.pdf(mean, Const.SONAR_STD, observedDist)
                self.belief.setProb(row,col,P*p)
    ...
    def elapseTime(self):
        ...
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        # P(Ct+1 | D1 = d1, …, Dt = dt)
        PCt1 = util.Belief(self.belief.getNumRows(),self.belief.getNumCols(),0.0)
        for oldTile, newTile in self.transProb:
            newP = self.belief.getProb(oldTile[0], oldTile[1]) * self.transProb[(oldTile,newTile)]
            PCt1.addProb(newTile[0], newTile[1], newP)
        PCt1.normalize()
        self.belief = PCt1
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    ##################################################################################
    # update |self.particles|={(row, col):num} with self.transProb[(oldTile, newTile)] & observedDist

    def observe(self, agentX, agentY, observedDist):
        # BEGIN_YOUR_CODE (our solution is 10 lines of code, but don't worry if you deviate from this)
        # Re-weight
        newPs = collections.defaultdict(int) 
        for tile in self.particles:
            # P(Ct ∣ D1 = d1, …, Dt−1= dt−1), No.of particles here
            P = self.particles[tile]
            mean = math.sqrt((agentX-util.colToX(tile[1]))**2 + (agentY-util.rowToY(tile[0]))**2)
            # P(dt | ct)
            p = util.pdf(mean, Const.SONAR_STD, observedDist)
            # P = P(Ct|...) * P(dt|ct)
            newPs[tile] = P*p
        ...
        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            newPar = util.weightedRandomChoice(newPs)
            self.particles[newPar] += 1
        # END_YOUR_CODE
    ...
    ##################################################################################
    # updated |self.particles| use
    # |self.transProbDict|={(oldTile):{transProbs},(oldTile):{transProbs},...}

    def elapseTime(self):
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        newPs = collections.defaultdict(int) 
        for tiles in self.particles:
            for _ in range(self.particles[tiles]):
                newPar = util.weightedRandomChoice(self.transProbDict[tiles])
                newPs[newPar] += 1
