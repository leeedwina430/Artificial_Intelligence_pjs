...
class PriorityQueue:
    ...
    def update(self, item, priority):
        ...
        for index, (p, c, i) in enumerate(self.heap):
            assert type(i) == node, 'i must be node'  # 验证i的type
...
class problem:
    ...
    def solution(self, node):
        ...
        path = []
        cost = node.path_cost
        while curnode.state != self.initial_state:
    ...
    def child_node(self, node_begin, action):
        ...
        """
        ################################# Your code here ###########################
        next_state = self.transition(node_begin.state, action)
        # 把路径的cost值相加
        cnode = node(
            state=next_state,
            parent=node_begin,
            path_cost=node_begin.path_cost + self.step_cost(node_begin.state, action, next_state),
            action=action)
        return cnode
