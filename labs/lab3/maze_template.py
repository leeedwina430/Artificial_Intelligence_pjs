...
class MazeEnv(object):
    ...
    def is_game_over(self, state):          # 是否到终止点
        return (state[0] == self.goal[0] and state[1] == self.goal[1])
    
    def transition(self, state:tuple, action:str):          # 转移矩阵
...
class MazeRLAgent(object):
    ...
    def policy_evaluation(self, env, values, policy):
        while True:
            ...
            for s in env.legel_states:
                output = env.transition(s, policy[s])   # [(1.0, next_state, reward, done)]
                new_values[s] = output[0][2] + self.gamma * values[output[0][1]] * (1-output[0][3])
                difference += new_values[s] - values[s]
            # ------ end ----------
    ...
    def policy_iteration(self, env):
        ...
        def chooseMax(s,values):
            max = -99999
            maxind = -1
            i = 0
            for action in env.legal_actions(s):
                output = env.transition(s, action)   # [(1.0, next_state, reward, done)]
                score = values[output[0][1]]
                if score > max:
                    max = score
                    maxind = i
                i += 1
            return env.legal_actions(s)[maxind]

        self.iteration_number = 0
        while True:

            # TODO: begin your code
            new_policy = {s:None for s in env.legel_states}
            self.iteration_number += 1

            new_values = self.policy_evaluation(env, values, policy)
            unchanged = True
            for s in env.legel_states:
                new_policy[s] = chooseMax(s,new_values)
    ...
    def value_iteration(self, env):
        """ VALUE iteration. """
        values = {s:0.0 for s in env.legel_states}      # 是一个dict
        ...
        def callNew(s):
            ...
            for action in env.legal_actions(s):
                output = env.transition(s, action)   # [(1.0, next_state, reward, done)]
                Reward = output[0][2]
                Usp = values[output[0][1]]
                score = Reward + self.gamma * Usp * (1-output[0][3])
                if score > max:
                    max = score
                    maxind = i
                i += 1
            return max, env.legal_actions(s)[maxind]
        ...
        while True:
            ...
            # TODO: begin your code
            for s in env.legel_states:
                new_values[s], policy[s] = callNew(s)
                difference += new_values[s] - values[s]
            # ------ end ----------
...
if __name__ == "__main__":
    while True:
        ...
        # print("Policy Iteration: {}".format(agent.iteration_number))
        for state in env.legel_states:
            print(f"{round(values[state], 4):.4f}", end=" ")
        print()
